...
import { environmentSlug } from "./environment.mjs";
import {
  ANSWER_FIELDS,
  ATTRIBUTION_FIELDS,
  CAMPAIGN_DIMENSIONS,
...
export const KEY_ROOT = "cs:v1";

/**
 * Keys are namespaced by logical environment, so preview traffic can never
 * land in the production dataset even if both were pointed at one database.
 * Resolved per call rather than cached, so tests can vary the environment.
 */
export function keyPrefix() {
  return `${KEY_ROOT}:${environmentSlug()}`;
}
...
export const keys = {
  ...
  unique: (event) => `${keyPrefix()}:uniq:${event}`,
  count: (event) => `${keyPrefix()}:count:${event}`,
  day: (day, event) => `${keyPrefix()}:day:${day}:uniq:${event}`,
  recent: () => `${keyPrefix()}:recent`,
  campaigns: () => `${keyPrefix()}:campaigns`,
  campaign: (key, event) => `${keyPrefix()}:camp:${key}:${event}`,
  answerValues: (field) => `${keyPrefix()}:answers:${field}`,
  answer: (field, value) => `${keyPrefix()}:ans:${field}:${value}`,
  rateLimit: (window, clientId) => `${keyPrefix()}:rl:${window}:${clientId}`,
};
...
export async function checkRateLimit(clientId) {
  ...
  const key = keys.rateLimit(window, clientId);
  try {
